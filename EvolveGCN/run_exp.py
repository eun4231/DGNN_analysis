...
if __name__ == '__main__':
	...
	if (args.start_profiling):
		with profile(activities=[ProfilerActivity.CPU, ProfilerActivity.CUDA], on_trace_ready= torch.profiler.tensorboard_trace_handler('./log/'+ 'GPU_EolveGCN_h'), 
				record_shapes = True, profile_memory = True, with_stack = True) as prof:

	#build the dataset
			with record_function("BUILD DATASET"):
				dataset = build_dataset(args)
			#build the tasker
			with record_function("BUILD TASKER"):
				tasker = build_tasker(args,dataset)
			#build the splitter
			with record_function("BUILD SPLITTER"):
				splitter = sp.splitter(args,tasker)
			#build the models
			with record_function("BUILD GCN"):
			...
			#trainer
			with record_function("BUILD TRAINER"):
