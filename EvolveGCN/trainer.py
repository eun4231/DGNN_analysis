...
class Trainer():
	def __init__(self,args, splitter, gcn, classifier, comp_loss, dataset, num_classes):
		...
		self.start_profiling = args.start_profiling
		self.profile_part = args.profile_part
	...
	def train(self):
		...
		for e in range(1):
			#print(self.profile_part == 'VALID')
			if(self.profile_part == 'TRAIN'):
				eval_train, nodes_embs = self.run_epoch(self.splitter.train, e, 'TRAIN', grad = False)
			if(self.profile_part == 'VALID'):
				eval_train, nodes_embs = self.run_epoch(self.splitter.dev, e, 'VALID', grad = False)
			if(self.profile_part == 'TEST'):
				eval_train, nodes_embs = self.run_epoch(self.splitter.test, e, 'TEST', grad = False)
			if(self.profile_part == 'TRAINING'):
	...
	def run_epoch(self, split, epoch, set_name, grad):
		...
		for s in split:
			...
			if (self.start_profiling):
				#print("I'm executing this")
				with record_function("PREPARE SAMPLE"):
					if self.tasker.is_static:
		...
		if (self.start_profiling):
			with record_function("EVAL MEASURE"):
