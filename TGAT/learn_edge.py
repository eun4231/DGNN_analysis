...
parser.add_argument('--use_cuda', type=str, choices=['True', 'False'], default='True', help='choose to use GPU or CPU')
...
NUM_TEST_BATCH = args.n_batch
START_PROFILING = args.start_profiling
...
def eval_one_epoch(hint, tgan, sampler, src, dst, ts, label):
    ...
    with torch.no_grad():
        ...
        if(FULL_DATASET == 'true'):
            ...
            print("the number of test batcg is:",num_test_batch)
        #num_test_batch = 25
        else:
            num_test_batch = NUM_TEST_BATCH
            print(" the number of test batch is:", num_test_batch)
...
train_label_l = label_l[valid_train_flag]  # select the training dataset
...
if (START_PROFILING):
    with profile(activities=[
        ...
        with record_function("model_initialization"):
            #device = torch.device('cuda:{}'.format(GPU))
            #device = 'cpu'
            tgan = TGAN(args, train_ngh_finder, n_feat, e_feat,
            ...
            num_instance = len(train_src_l)  #number of instances in the training set 81029
    ...
    if(DATASET_TYPE == 'Train'):
        train_acc, train_ap, train_f1, train_auc = eval_one_epoch('train for old nodes', tgan, train_rand_sampler, train_src_l, 
        train_dst_l, train_ts_l, train_label_l)        
